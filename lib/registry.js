...
var debug = require('debug')('enrouten:registry');
...
module.exports = function create(mountpath, router, options) {
    ...
    // Attach user specified configuration to the router for the ability to pass
    // inject configuration into the route creation.
    if (options.routerUserConfig && !router.userConfig) {
        Object.defineProperty(router, 'userConfig', {
            enumerable: false,
            configurable: false,
            writable: false,
            value: options.routerUserConfig
        });
    }
